...
/**
 * PENALTY GRID COMPONENT - Red text moved to bottom right
 */
function PenaltyGrid() {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
      {penalties.map((p, i) => (
        <div key={i} className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex flex-col justify-between min-h-[100px]">
          <div>
            ...
            {navItems.map((item) => (
              <button
                ...
                className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-full transition-all text-sm sm:text-base ${
                  activeTab === item.id ? 'bg-amber-500 text-slate-900 font-bold' : 'hover:bg-slate-800 text-slate-300'
                }`}
              >
      ...
      <main className="max-w-6xl mx-auto p-4 sm:p-6">
        {activeTab === 'website' && (
          <div className="animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-8 sm:mb-12">
              <StatBox label="Max Power" value="250W" /><StatBox label="Speed Cut" value="25 km/h" /><StatBox label="Standard" value="EN 15194" /><StatBox label="Penalty" value="Seizure" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
              <div className="lg:col-span-2 space-y-8 sm:space-y-10">
                <section>
                  ...
                  <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-200">
                    <p className="mb-4 text-slate-700 leading-relaxed text-sm sm:text-base">Every e-bike used on NSW roads or shared paths must be an <strong>EPAC</strong>. Legality is determined by the manufacturer's label.</p>
                ...
                <section>
                  ...
                  <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-200">
                    <div className="flex items-start gap-4 p-4 bg-red-50 border-l-4 border-red-600 rounded-r-xl mb-6 text-sm">
                      <AlertCircle className="text-red-600 shrink-0" />
                      <p className="text-sm text-red-900 font-bold leading-snug">Under the Road Transport Act 2013, illegal bikes are destroyed (crushed). Police use portable <strong>Dyno Units</strong> to test wattage on-the-spot.</p>
                    </div>
                    <a href={LINKS.FAIR_TRADING} target="_blank" rel="noreferrer" className="block w-full py-3 bg-slate-900 text-white rounded-xl text-center font-bold text-xs uppercase tracking-widest hover:bg-slate-800 transition-all">Report Illegal Retailers to Fair Trading</a>
                  </div>
                </section>
              </div>
              ...
              <div className="space-y-6">
                <LegalityQuiz />
                <div className="bg-slate-900 text-white p-6 sm:p-8 rounded-3xl shadow-xl border-t-4 border-white">
                  <h3 className="text-2xl font-black mb-4 text-white uppercase tracking-tighter leading-none">Is your house at risk?</h3>
                  <p className="text-white text-sm leading-relaxed mb-6 italic">If an underage rider causes injury on an illegal e-bike, the financial fallout is catastrophic.</p>
                  <div className="space-y-6">
            ...
            <footer className="mt-12 sm:mt-20 border-t border-slate-200 pt-10 pb-20 text-sm print:hidden">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4">
                  ...
                  <div className="flex flex-col gap-2 font-bold text-blue-600">
                    <a href={LINKS.ROAD_RULES} target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:underline"><ExternalLink size={14}/> Road Rules 2014 (NSW)</a>
                    <a href={LINKS.ROAD_TRANSPORT_ACT} target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:underline"><ExternalLink size={14}/> Road Transport Act 2013 (NSW)</a>
                    <a href={LINKS.VEHICLE_STANDARDS_ACT} target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:underline"><ExternalLink size={14}/> Road Vehicle Standards Act 2018 (Cth)</a>
                  </div>
                ...
                <div>
                  ...
                  <div className="space-y-3 font-bold text-blue-600">
                    <a href={LINKS.TfNSW_EBIKES} target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:underline"><ExternalLink size={14}/> TfNSW: E-bike Safety & Sticker Info</a>
                    <a href={LINKS.FAMILY_ADVICE_PDF} target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:underline"><ExternalLink size={14}/> Advice for families (Official PDF)</a>
                  </div>
                </div>
              </div>
            </footer>
        ...
        {activeTab === 'flyer' && (
          <div className="flex flex-col items-center py-6 sm:py-10 animate-in fade-in zoom-in-95">
            <a href={LINKS.PDF_DOWNLOAD} download className="mb-8 flex items-center gap-3 bg-slate-900 text-white px-10 py-4 rounded-full font-black uppercase text-base hover:scale-105 transition-transform shadow-2xl">
            ...
            <div className="print-container bg-white shadow-2xl overflow-hidden border-[6px] sm:border-[12px] border-slate-900 origin-top scale-[0.4] sm:scale-[0.55] md:scale-75 lg:scale-100 transition-transform mb-[-600px] sm:mb-[-500px] md:mb-[-150px] lg:mb-10" 
                ...
                <div className="bg-[#C0392B] text-white text-center font-bold py-3 mb-6 text-lg uppercase leading-tight">CRACKDOWN ACTIVE: NON-COMPLIANT BIKES ARE SUBJECT TO SEIZURE AND DESTRUCTION.</div>
                <div className="space-y-6 text-slate-800">
                  <section className="border-b-2 border-[#1A2A3A] pb-1"><h3 className="text-xl font-black text-[#1A2A3A] uppercase tracking-tighter leading-none pb-1">1. The Compliance Sticker Rule</h3></section>
                  <div className="grid grid-cols-1 gap-2 text-sm font-medium"><p>• <strong>The 250W Cap:</strong> Motor power must not exceed 250W.</p><p>• <strong>Mandatory Labeling:</strong> Permanent EN 15194 compliance sticker must be visible.</p><p>• <strong>Rating {'>'} 250W:</strong> Legally a <strong>Motorcycle</strong>. Illegal for unlicensed minors.</p></div>
                  <section className="border-b-2 border-[#1A2A3A] pb-1 pt-2"><h3 className="text-xl font-black text-[#1A2A3A] uppercase tracking-tighter leading-none pb-1">2. Seizure {'&'} Retailer Accountability</h3></section>
                  <div className="grid grid-cols-1 gap-2 text-sm font-medium leading-tight"><p>• <strong>No Sticker = Seizure:</strong> Bikes lacking compliance stickers are presumed illegal.</p><p>• <strong>Retailer Warning:</strong> Selling high-power bikes as "legal" is a breach of Consumer Law.</p><p>• <strong>Dyno Testing:</strong> Police use portable units to test wattage roadside.</p></div>
                  <div className="grid grid-cols-2 gap-4 pt-4">
                    <div className="bg-[#F4F7F6] p-4 rounded-lg border border-[#1A2A3A]"><h4 className="font-black text-xs uppercase mb-2">Penalties at a Glance</h4><div className="text-[10px] space-y-1 font-bold"><p>Unregistered Vehicle: $700+ Fine</p><p>Uninsured Vehicle: $700+ Fine</p><p>TfNSW Seizure Fee: $1,200+</p></div></div>
                    <div className="bg-white p-4 rounded-lg border-2 border-[#C0392B]"><h4 className="font-black text-xs text-[#C0392B] uppercase mb-2">Parents: Asset Risk</h4><div className="text-[10px] space-y-1 font-bold"><p>Insurance: Policy VOID for motorcycles.</p><p>Liability: Millions in medical costs.</p><p>House: Assets can be seized to pay debts.</p></div></div>
                  </div>
                  <section className="border-b-2 border-[#C0392B] pb-1 pt-2"><h3 className="text-xl font-black text-[#C0392B] uppercase tracking-tighter leading-none">Parents: Is Your Home At Risk?</h3></section>
                  <div className="grid grid-cols-1 gap-1 text-[11px] font-bold leading-snug bg-red-50 p-2 border border-red-200 rounded"><p>1. <strong>NO INSURANCE:</strong> Home policies exclude unregistered motor vehicles.</p><p>2. <strong>PERSONAL LIABILITY:</strong> Parents sued for millions in medical damages.</p><p>3. <strong>ASSET LIQUIDATION:</strong> Savings and the family home seized to pay debts.</p></div>
                </div>
