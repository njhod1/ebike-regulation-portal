...
- `realRisk` (optional) — `{ stat, detail, url }`, `url` optional. Rendered by `RealRiskSection.jsx` on every state page, above `keyDates`; the national baseline text (NRMA's 15+ deaths in 2025, a University of Melbourne study's 37 fatal e-scooter incidents 2020–2025, the AMA's injury description) is hardcoded there since it's the same fact everywhere. `realRisk` adds one state-specific highlight on top, sourced to official/health-department data where available (NSW: Transport for NSW; VIC: Monash Accident Research Centre; QLD: Queensland Health, tabled in Parliament; WA: the parliamentary Ride Safe inquiry; SA: SA Police via RAA; ACT: Canberra Hospital vs. official ED-coding figures) — present on 7 of 8 states. **TAS is deliberately absent, not missed**: a research pass specifically looking for TAS-equivalent data found no credible state-specific aggregate (a relevant Royal Hobart Hospital trauma study exists but its actual figures were never confirmed, and the only other lead was a single 2025 death with no aggregate context) — the section falls back to the national baseline alone for TAS, which is the honest answer, not a padded or weak stat. If you ever do find solid TAS-specific data, this is exactly the kind of gap `realRisk` exists to fill.
